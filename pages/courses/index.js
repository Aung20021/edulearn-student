...
export default function Course() {
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        page: page.toString(),
        sort,
        isPaid: isPaid !== "" ? isPaid : undefined, // Include isPaid filter if selected
  ...
  const fetchSuggestions = async (term) => {
    ...
    const response = await fetch(
      `/api/courses?search=${term}&page=1&published=true`
    );
    if (response.ok) {
      const data = await response.json();
          ...
          >
            ...
            >
              ...
              <p className="mt-2 max-w-2xl text-gray-600 text-sm sm:text-base">
                Explore a variety of courses designed to enhance your learning
                experience.
              </p>
            </motion.div>
